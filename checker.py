...
def main():
    ...
    ids_data = []
    if os.path.exists("ids.csv"):
        with open("ids.csv", 'r', encoding='utf-8') as f:
            import csv
    ...
    print(f"Starting check for {len(ids_data)} IDs using the FREE Hacienda API...")
    print("This is completely free and requires no API key!")
    print("-" * 50)

    for i, row in enumerate(ids_data, 1):
        person_id = row.get("ID", "").strip()
        hour = row.get("Time", "").strip()
        average = row.get("Average", "").strip()
        print(f"[{i}/{len(ids_data)}] Checking ID {person_id}...", end="\r", flush=True)
        try:
                ...
                if response.status == 200:
                    ...
                    if full_name:
                        ...
                        for target_name in normalized_target_names:
                            ...
                            if all(
                                part in normalized_full_name.split()
                                for part in target_parts
                            ):
                                matched = True
                                match_counter += 1
                                print(f"[MATCH] Pos: {i} | Match #{match_counter} | Name: {full_name} | ID: {person_id} | Hour: {hour} | Avg: {average}".ljust(120))
                                break
                    else:
